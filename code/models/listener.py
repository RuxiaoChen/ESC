...
class CopyListener(nn.Module):
    def __init__(self, feat_model, message_size=100, dropout=0.2):
        ...
        # self.classl1 = nn.Linear(512, 128)
        # self.classl2 = nn.Linear(128, 10)
    ...
    def compare(self, feats_emb, message_enc):
        ...
        # message_enc_expanded = message_enc.unsqueeze(1).expand_as(feats_emb)
        # # element-by-element multiplication
        # result = feats_emb * message_enc_expanded
        # output_vector = self.classl1(result.view(-1, 512))
        # output_vector = self.classl2(output_vector)
        # # change the oput vector shape to [1, 5, 10]
        # scores = output_vector.view(1, 5, 10)

        # for referential game
