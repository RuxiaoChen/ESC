...
class CopySpeaker(nn.Module):
    ...
    def embed_features(self, feats, targets=None):
        ...
        feats_emb = feats_emb_flat.unsqueeze(1).view(batch_size, n_obj, -1)   # 5,5,512

        if targets is None:
            feats_emb_dropout = self.dropout(feats_emb)    # 5, 1, 512
            feats_emb_dropout = feats_emb_dropout.squeeze(1)
            return feats_emb_dropout
        else:
            prototypes = self.form_prototypes(feats_emb, targets)   # 5, 1024
...
class Speaker(CopySpeaker):
    ...
    def forward(self, feats, targets, **kwargs):
        ...
        states = self.my_init_h(feats_emb)
