...
def run(epsil,
    ...
    for batch_i, batch in enumerate(dataloader):
        ...
        if args.listener_only:
            ...
            # Channel Simulation
            lang=message_in_channel2(lang,epsil)

            # Lister process the received message
        ...
        if args.reference_game_xent:
            ...
            this_loss = pair.bce_criterion(lis_scores, lis_y)    # # Calculate the binary cross-entropy loss
            lis_pred = (lis_scores > 0).float()  # Elements greater than 0 become 1, elements less than or equal to 0 become 0, and the result is converted to floating point.
            # # print(lis_pred)
            per_game_acc = (lis_pred == lis_y).float().mean(1).cpu().numpy()   # Calculate whether the predicted value lis_pred is equal to the label lis_y.
            # this_acc = per_game_acc.mean()
            # per_game_acc = 'nan'
            max_indices = torch.argmax(lis_scores, dim=1)
            lis_pred = torch.zeros_like(lis_scores)
            # Set the element at the index of the maximum value of each row to 1
...
if __name__ == "__main__":
    ...
    # whether to use the trained model as a start or use the trained model to do the evaluation
    checkpoint = torch.load('exp/exp/v6_l21_n010_po/best_model.pt')
    ...
    for epoch in range(args.epochs):
        ...
        # Train. when training, enabling the line below, Comment out the code from "test start" to "test end"
        # train_metrics, lang = run(0.3, 'train', epoch, *run_args)    # the first parameter is message error rate
            ...
            for split in ["val", "test"]:
                ...
                for split_type in ["", "_same"]:
                    ...
                    if sname in dataloaders:
                        eval_metrics, eval_lang = run(0.0, sname, epoch, *run_args)
